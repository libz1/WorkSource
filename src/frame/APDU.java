...
public class APDU {
	...
	public APDU(String svr_priority,String svr_no) {
		PIID_PRI = DataConvert.String2Int(svr_priority);
		PIID_NO = DataConvert.String2Int(svr_no);
	}
	...
	public void init(String data) {
		...
		if (choiseFlag == 129) {
			setPIID(data.substring(2, 4));
			setNextData(data.substring(4, data.length()));
			String result = nextData.substring(0, 2);
			result = DataConvert.hexString2BinString(result,8);

			try{
				nextData.substring(0, 2);
				node = new TreeNode("结果", "0x"+result);
				dataNode.addChild(node);

				String tmp = DataConvert.hexString2BinString(result, 8);
				TreeNode child = null;
				if (tmp.substring(0).equals("1"))
					child = new TreeNode("时钟可信标志", "1-可信");
				else
					child = new TreeNode("时钟可信标志", "0-可信");
				node.addChild(child);
				int tmp_val = DataConvert.BinStrToInt(tmp.substring(5,8));
				if (tmp_val == 0)
					child = new TreeNode("结果", "0-成功");
				else if (tmp_val == 1)
					child = new TreeNode("结果", "1-地址重复");
				else if (tmp_val == 2)
					child = new TreeNode("结果", "2-非法设备");
				else if (tmp_val == 3)
					child = new TreeNode("结果", "3-容量不足");
				else
					child = new TreeNode("结果", tmp_val);
				node.addChild(child);

				String time = nextData.substring(2, 2+20);
				time = Util698.BCDDateTimeToNormal(time);
				node = new TreeNode("请求时间", time);
				dataNode.addChild(node);
				time = nextData.substring(22, 22+20);
				time = Util698.BCDDateTimeToNormal(time);
				node = new TreeNode("收到时间", time);
				dataNode.addChild(node);
				time = nextData.substring(22+20, 22+20*2);
				time = Util698.BCDDateTimeToNormal(time);
				node = new TreeNode("响应时间", time);
				dataNode.addChild(node);
...
//				  { 一个目标服务器地址         TSA，	代理一个服务器的超时时间   long-unsigned，	若干个对象属性描述符       SEQUENCE OF OAD//					  }					}
				...
				if (choiseFlag_1 == 2) {
					...
					s = new AnalyData().dealRecord(s[1], dataNode);
//					dealRecord
//					s = new AnalyData().dealOAD(s[1], dataNode);
//					s = new AnalyData().dealRSD(s[1], dataNode);
//					s = new AnalyData().dealRCSD(s[1], dataNode);
				}
				if (choiseFlag_1 == 3) {
					node = new TreeNode("整个代理请求的超时时间"+nextData.substring(0,4), DataConvert.hexString2Int(nextData.substring(0,4)));
					dataNode.addChild(node);
		...
		if (choiseFlag == 137) {
			...
			try{
				...
				if (choiseFlag_1 == 2) {
					...
					s = new AnalyData().dealResultRecord(s[1], dataNode);
...
//					透明转发命令返回结果     TransResult
					s = new AnalyData().dealOAD(nextData, dataNode);
					s = new AnalyData().dealTransResult(s[1], dataNode);
				}
				s = new AnalyData().dealTimeTag(s[1], dataNode);
