...
public class Frame698 {
	...
	public Frame698(String APDU,int type){
		// 1、设置终端地址
		getFrameAddr().setSAData(SoftParameter.getInstance().getSendTerminal());
		// 2、设置APDU信息
		getAPDU().init(APDU);
		// 3、设置通信相关参数  在Frame698.FrameContro的构造函数中，已经设置为默认参数了
		getFrameControl().setDIRFlag("0");
		getFrameControl().setPRMFlag("1");
		getFrameControl().setFunData(3);
...
//		System.out.println("Json数据-链路报文:\r\n"+analy_userdata.convetToString());
		...
		TreeNode tmp = analy_frame.find_child("服务器地址",true);

		TreeNode node = aPDU.dataNode;
		msg = node.loop(0);
		msg = tmp.getKey()+tmp.getValue()+"\r\n"+msg;

		// xuky 2017.06.05  调整添加的次序
		tmp = analy_frame.find_child("APDU子类型",false);
		msg = tmp.getKey()+tmp.getValue()+"\r\n"+msg;

		tmp = analy_frame.find_child("APDU类型",false);
		msg = tmp.getKey()+tmp.getValue()+"\r\n"+msg;
