...
public class FrameNew {
	...
	static Boolean With_Root = false;
	static String type_old = "";
...
//		System.out.println("FrameNew=>"+frame);
	...
	public void setObjectData(Object object,int code){
		frame = getAPDUData(code,object);
	}
	...
	private String getAPDUData(int code,Object object){
		...
		frame = new AnalyData().getAPDUData(data_val_new,-1,false);
		ret = frame;
	...
	private static Object[][] getFuncParas(int code,Object object,Boolean WithRoot,String type){
		...
		// xuky 2017.03.09 使用如下的代码有问题， 如果进行此效率的提高，导致异常，解析档案报文和电表档案同步先后进行执行时会出现错误
		// xuky 2017.03.09 19:33 发现bug，addArray的入参条件有5个，其中与三个的变化会影响数据，必须全面考虑这些因素的影响
		if (Data_val == null || Data_val_Code != code || WithRoot != With_Root || type_old != type ) {
//			System.out.println("getFuncParas=>code:"+code + "  WithRoot:"+WithRoot+ " type:"+type);
...
//		System.out.println("getFuncParas=>type:"+Data_val[0][2].getClass().getTypeName());
...
//		Object[][] data_val = Data_val;
		...
		// xuky 2017.03.09 进行data_val = Data_val操作后，对data_val的修改会影响到Data_val
		// 所以改为使用数组赋值代码，进行数据的重复复制
		Object[][] data_val = null;
		data_val = Util698.setArrayData(Data_val);
...
//					System.out.println(rowNum + " getObject=> type"+type +" typeData " + typeData);
					...
					String[] d = analyData.dealType10(temp, node);
					typeData = analyData.dealTreeNode(node);
					temp = d[1];
//					System.out.println("typeData-"+typeData+" temp-"+temp );
				}
				else{
				...
				if (t[0].indexOf("datetimeBCD")>=0){
					// 复用AnalyData中的数据类型解析函数
					TreeNode node = new TreeNode();
					AnalyData analyData = new AnalyData();
					analyData.dealType28(typeData, node);
					typeData = analyData.dealTreeNode(node);
				}
				temp = temp.substring(byteLen*2);
//				System.out.println(rowNum + " getObject=> type"+type +" typeData " + typeData);
...
//				System.out.println(" getObject=> type"+type+" attr "+attr +" val " + typeData);
	...
	public Object[][] getDataValsToDeal() {
		return DataValsToDeal;
	}
