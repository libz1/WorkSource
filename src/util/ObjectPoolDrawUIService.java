...
import javafx.assident.DrawUIService;

// 对象池 xuky 2018.03.22
// 参考http://blog.csdn.net/shimiso/article/details/9814917
// 调整代码Bug，调整为整体的单例对象
public class ObjectPoolDrawUIService {
	private volatile static ObjectPoolDrawUIService uniqueInstance;
	...
	public static ObjectPoolDrawUIService getInstance() {
		if (uniqueInstance == null) {
			synchronized (ObjectPoolDrawUIService.class) {
				if (uniqueInstance == null) {
					// 双重检查加锁
					uniqueInstance = new ObjectPoolDrawUIService();
				}
			}
		}
	...
	private ObjectPoolDrawUIService() {
		createPool();
	}
	...
	private Object findFreeObject() {
		...
		while (enumerate.hasMoreElements()) {
			...
			if (!pObj.isBusy()) {
				...
				obj = pObj.getObject();
				pObj.setBusy(true);
//				System.out.println("ObjectPool1 找到可用对象"+i );
